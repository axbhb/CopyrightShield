...
from composition_attack_load_dataset import load_poisoned_dataset, preprocess_train_silentbaddiffusion, collate_fn_silentbaddiffusion, SPEC_CHAR, DATASET_NAME_MAPPING
import functools
...
model_cards = {
    'stabilityai1': ('stabilityai/stable-diffusion-2-base',768),
    'stabilityai2': ('D:\SilentBadDiffusion-master\CompVis\stable-diffusion-2',768),
    'stabilityai3': ('stabilityai/stable-diffusion-2-1',768),
    'CompVis1': ('D:\SilentBadDiffusion-master\CompVis\stable-diffusion-v1-1',512),
    'CompVis2': ('D:\SilentBadDiffusion-master\CompVis\stable-diffusion-v1-2',512),
    'CompVis3': ('D:\SilentBadDiffusion-master\CompVis\stable-diffusion-v1-3',512),
    'CompVis4': ('D:\SilentBadDiffusion-master\CompVis\stable-diffusion-v1-4',512),
    'CompVis5': ('D:\SilentBadDiffusion-master\CompVis\stable-diffusion-v1-5',512),
    'finetune':('')
}
...
def main(args, poisoned_dataset, tgt_img_path_list, tgt_caption_list, tgt_phrases_list):    
    ...
    if SilentBadDiffusion_modification:
        ...
        train_dataloader = torch.utils.data.DataLoader(
            poisoned_dataset,
            shuffle=True,
            collate_fn = lambda x: collate_fn_silentbaddiffusion(x, batch_size=4),
            batch_size=args.train_batch_size,
            num_workers=args.dataloader_num_workers,
        )
        # collate_fn = lambda x: collate_fn_silentbaddiffusion(x, batch_size=8),
        if args.add_loss is True:
            class DetectDataset(torch.utils.data.Dataset):
                ...
                def __getitem__(self, idx):
                    ...
                    idx_value = torch.tensor(idx)

                    return {"pixel_values": image, "input_ids": input_ids, "idx": idx_value}

            detect_dataset = DetectDataset(args.detect_path, transform=train_transforms)


            def collate_fn1(examples, batch_size):
                ...
                if current_batch_size < batch_size:
                    ...
                    missing_count = batch_size - current_batch_size

                    _, c, h, w = pixel_values.shape

                    padding_pixel_values = torch.zeros((missing_count, c, h, w), dtype=pixel_values.dtype)
                    pixel_values = torch.cat([pixel_values, padding_pixel_values], dim=0)

                    max_length = input_ids.size(1)
                    padding_input_ids = torch.zeros((missing_count, max_length), dtype=input_ids.dtype)
                    input_ids = torch.cat([input_ids, padding_input_ids], dim=0)

                    padding_idx = torch.zeros(missing_count, dtype=idx.dtype)
                    idx = torch.cat([idx, padding_idx], dim=0)

                return {"pixel_values": pixel_values, "input_ids": input_ids, "idx": idx}

            # 创建 DataLoader
            detect_dataloader = torch.utils.data.DataLoader(detect_dataset, args.train_batch_size, shuffle=True,
                                                            collate_fn=lambda x: collate_fn1(x, batch_size=4),
                                                            num_workers=args.dataloader_num_workers)
            detect_dataloader, train_dataloader = accelerator.prepare(detect_dataloader , train_dataloader)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.add_loss is True:
                    ...
                    num_batches = 0
                    for batchh in detect_dataloader:
                        images = batchh["pixel_values"].to(weight_dtype)
                        with torch.no_grad():
                            poison_latents = vae.encode(images.to(weight_dtype)).latent_dist.sample()
                            poison_latents = poison_latents * vae.config.scaling_factor
                        poison_noisy_latents = noise_scheduler.add_noise(poison_latents, noise, timesteps)
                        similarity_loss = -F.mse_loss(model_pred.float(), poison_noisy_latents.float(), reduction="mean")
                        total_similarity_loss += similarity_loss.item()
                        num_batches += 1

                    average_similarity_loss = total_similarity_loss / num_batches
                    lambda_poison = 0.05
                    loss = loss + lambda_poison * average_similarity_loss
                    loss = loss.mean()
...
def add_SilentBadDiffusion_args(parser):
    ...
    parser.add_argument("--log_save_path", type=str, default= 'F:\style_defense/')
    ...
    parser.add_argument("--dataset_name", type=str, default="Midjourney", help='Pokemon / Midjourne / style/ DDB')
    parser.add_argument("--clean_dataset_name", type=str, default='COYO', help='Pokemon, COYO, LAION,/style/DDB')
    parser.add_argument("--target_start_id", type=int, default=68, help='the id of the target image')
    ...
    parser.add_argument("--n_few_shot", type=int, default=1)#1
    ...
    parser.add_argument("--detect_path", type=str, default=r'D:\SilentBadDiffusion-master\datasets\Midjourney\poisoning_images\68')
    return parser
...
if __name__ == "__main__":
    ...
    if SilentBadDiffusion_modification:
        ...
        args.log_save_path =  args.log_save_path
